...
exports.getAllPosts = (req, res, next) => {
    ...
    connect.query(sqlGetPosts, [userID, userID, userID], function (err, result) {
        if (err) {
...
exports.getOnePost = (req, res, next) => {
    ...
    connect.query(sqlGetPost, [userID, userID, userID, postID, postID], function (err, result) {
        if (err) {
            return res.status(500).json(err.message);
        }
        if (result.length == 0) {
...
exports.createPost = (req, res, next) => {
    ...
    const gifUrl = `${req.protocol}://${req.get("host")}/images/${req.file.filename}`; // ATTENTION A VERIFIER AVEC LE FRONT  NE FONCTIONNE PAS POUR L'INSTANT (D'OU LA LIGNE 72)
    console.log(gifUrl);
    ...
    connect.query(sqlCreatePost, values, function (err, result) {
        if (err) {
            return res.status(500).json(err.message);
        }
